...
# Note: << makes @where clause a collection, so the build_for methods below becomes :
# @where_clause(case_insentive_search(:first_name) OR #{case_insentive_search(:last_name)}), @where_args[:first_name = starts_with(seach_term), :last_name = starts_with(search_term)]
